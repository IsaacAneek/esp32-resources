# ESP32 Resources
## Memory
 - [ESP32 Programmers’ Memory Model](https://developer.espressif.com/blog/esp32-programmers-memory-model/)
 - [ESP32 Memory Organization](https://deepwiki.com/evilbinary/esp32-hello/2.2-memory-organization)
 - [Understanding ESP32 Memory](https://deepwiki.com/evilbinary/esp32-hello/2.2-memory-organization)
 - [ESP32 Memory types](https://www.esp32.ai/idf/esp32/api-guides/memory-types)
## Partition Table
 - [Partition tables](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-guides/partition-tables.html)
 - [Partition tables and the Bootloader of the ESP32](https://github.com/iqnev/Partition_tables_and_Bootloader_of_the_ESP32)
 - [Partition table](https://espressif-docs.readthedocs-hosted.com/projects/arduino-esp32/en/latest/tutorials/partition_table.html)
## Bootloader
 - [Understanding ESP32 boot process](https://circuitlabs.net/understanding-esp32-boot-process/)
## OTA (Over The Air) Firmware update
 - [Over The Air (OTA) Updates](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-reference/system/ota.html)
 - [ESP32 Native OTA Example (HTTPS)](https://github.com/espressif/esp-idf/blob/master/examples/system/ota/native_ota_example/main/native_ota_example.c)
## DAC
 - [Digital to Analog Converter (DAC)](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-reference/peripherals/dac.html)
 - [ESP32 DAC Audio Analog Output With Examples (Arduino IDE)](https://deepbluembedded.com/esp32-dac-audio-arduino-examples/)
 - [Built-In Digital-To-Analog Converter (DAC) With High Speed Direct Memory Access (DMA)](https://done.land/components/microcontroller/families/esp/espfeatures/dac/)
 - [ESP32 DAC Examples](https://github.com/espressif/esp-idf/tree/v5.5.1/examples/peripherals/dac)
 - ### Interesting readings
   - [How to Make and Test a Better DAC With ESP32](https://www.instructables.com/How-to-Make-and-Test-a-Better-DAC-With-ESP32/)
## I2S Protocol
 - [I2S Protocol : Working, Differences & Its Applications](https://www.elprocus.com/i2s-protocol/)
 - [Inter-IC Sound (I2S)](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-reference/peripherals/i2s.html)
 - [ESP32 - Intro to I2S Episode 1, explanation with basic example outputting a square wave](https://youtu.be/GFg1ve9tcSE?si=WwlHGD37OVdCMfAy)
 - [Chapter 138: I2S Audio Interface of ESP32](https://circuitlabs.net/i2s-audio-interface-of-esp32/)
 - [Master Efficient ADC Data Collection with ESP32 DMA and I2S | The Complete Guide](https://saludpcb.com/adc-collection-esp32-dma-i2s-complete-guide/)
## DMA
 - [Chapter 244: Advanced DMA Operations in ESP32](https://circuitlabs.net/advanced-dma-operations-in-esp32/)
## Audio
 - Digital Audio Basics
   - [Digital audio basics: audio sample rate and bit depth](https://www.izotope.com/en/learn/digital-audio-basics-sample-rate-and-bit-depth.html)
   - [Understanding Digital Audio: Sampling, Quantization, and More](https://www.theaudiopod.com/blog/xank7qi4qjlgxsltcwz3wdkzjdnagy)
   - [Introduction to audio data](https://huggingface.co/learn/audio-course/chapter1/audio_data)
 - Pulse Code Modulation (PCM)
   - [Pulse-code Modulation](https://en.wikipedia.org/wiki/Pulse-code_modulation)
 - LM386
   - [LM386 Datasheet](https://www.ti.com/lit/ds/symlink/lm386.pdf?ts=1734118207298)
   - [Tales From the Chip: LM386 Audio Amplifier](https://www.instructables.com/Tales-From-the-Chip-LM386-Audio-Amplifier/)
   - [Build a Great Sounding Audio Amplifier (with Bass Boost) from the LM386](https://www.circuitbasics.com/build-a-great-sounding-audio-amplifier-with-bass-boost-from-the-lm386/)
 - WAV
   - [WAVE PCM Sound File Format](http://soundfile.sapp.org/doc/WaveFormat/)
 - Interfacing with ESP32
   - [ESP32 Audio: I2S & Built-In DACs Explained](https://www.atomic14.com/videos/posts/lgDu88Y411o)
   - [Working with ESP32 Audio Sampling](https://www.toptal.com/embedded/esp32-audio-sampling)
   - [Stream Your Audio on the ESP32](https://www.hackster.io/julianfschroeter/stream-your-audio-on-the-esp32-2e4661)
   - [How to sample Audio on ESP32 I2S - Tutorial](https://youtu.be/ca2GKE4xkvM?si=Kv2aiwagEYehgC9W)
   - [ESP32 Audio DMA settings explained - dma_buf_len and dma_buf_count](https://youtu.be/ejyt-kWmys8?si=QPw32i9id63ucwvO)
   - [ESP32 Audio: I2S & Built-in DACs Explained](https://youtu.be/lgDu88Y411o?si=sac3ZZOf8bhp8OMC)
 - Misc.
   - [File to C style array converter](https://notisrac.github.io/FileToCArray/)
   - [Convert audio to WAV](https://audio.online-convert.com/convert-to-wav)
